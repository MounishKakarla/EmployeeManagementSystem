...
table{width:100%;border-collapse:collapse;font-size:11px}th{background:#4361ee;color:#fff;padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.5px}
td{padding:7px 10px;border-bottom:1px solid #eee;vertical-align:middle}
...
<strong>Week:</strong> ${dayjs(t.weekStartDate).format('DD MMM YYYY')} — ${weekEnd}<br>
...
<table><thead><tr><th>Project</th><th>Task</th><th style="text-align:center">Hours</th><th>Submitted</th></tr></thead>
<tbody><tr><td><strong>${t.project || '—'}</strong></td><td>${t.taskDescription || '—'}</td>
<td style="text-align:center;font-weight:700;color:#4361ee">${t.totalHours ?? 0}h</td>
<td style="color:#888;font-size:11px">${t.submittedAt ? dayjs(t.submittedAt).format('DD MMM YYYY') : '—'}</td></tr></tbody></table>
<div class="footer">Generated by Tektalis EMS &bull; ${dayjs().format('DD MMM YYYY, HH:mm')}</div>
</body></html>`
    try {
