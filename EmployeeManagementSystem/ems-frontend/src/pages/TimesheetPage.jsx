...
<p class="total">Total: ${totalH.toFixed(1)}h &nbsp;/&nbsp; Target: ${targetH}h</p>
<div class="footer">Generated by Tektalis EMS &middot; ${new Date().toLocaleString()}</div>
...
<p class="sub">Generated by ${user?.name || 'Admin'} · Tektalis EMS</p>
...
<div class="footer">Generated by Tektalis EMS · ${new Date().toLocaleString()}</div>
<script>window.onload=()=>setTimeout(()=>window.print(),400)</script>
</body></html>`
          ...
          <TimesheetTable records={teamHistory} isLoading={teamLoading || teamFetching}
            ...
            actions={(r) => {
              const exportSingle = () => {
                ...
                const statusFg = r.status === 'APPROVED' ? '#155724' : r.status === 'SUBMITTED' ? '#856404' : r.status === 'REJECTED' ? '#721c24' : '#555'
                const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
<title>Timesheet — ${empId}</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;padding:36px;color:#111;font-size:13px}
h1{font-size:20px;margin-bottom:6px;color:#1a1a2e}.meta{color:#555;font-size:12px;margin-bottom:24px;line-height:1.8}.meta strong{color:#111}
table{width:100%;border-collapse:collapse}th{background:#4361ee;color:#fff;padding:9px 12px;text-align:left;font-size:11px;text-transform:uppercase}
td{padding:8px 12px;border-bottom:1px solid #eee}.footer{margin-top:28px;font-size:10px;color:#aaa;border-top:1px solid #eee;padding-top:12px}
@media print{body{padding:16px}}</style></head><body>
<h1>Employee Timesheet</h1>
<div class="meta"><strong>Employee:</strong> ${r.employeeName || '—'} (${empId})<br>
<strong>Week:</strong> ${formatDate(r.weekStartDate)}<br>
<strong>Status:</strong> <span style="background:${statusBg};color:${statusFg};padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700">${r.status}</span></div>
<table><thead><tr><th>Project</th><th>Task</th><th>Total Hours</th><th>Submitted</th></tr></thead>
<tbody><tr><td><strong>${r.project || '—'}</strong></td><td>${r.taskDescription || '—'}</td>
<td style="font-weight:700;color:#4361ee">${r.totalHours ?? 0}h</td>
<td style="color:#888">${r.submittedAt ? formatDate(r.submittedAt) : '—'}</td></tr></tbody></table>
<div class="footer">Generated by Tektalis EMS · ${new Date().toLocaleString()}</div>
